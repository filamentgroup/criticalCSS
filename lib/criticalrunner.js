...
(function(){
	...
	var site = system.args[1],
		...
		initialMap = JSON.parse(system.args[5]),
		rulesFile = system.args[6],
		ignoreConsole = system.args.length > 7 && system.args[7] === "--ignoreConsole" ? true : false,
		contents, rules;
	...
	page.open( site, function( status ){
		...
		// map the rules with fixed initial values when we have a mapping
		crit = crit.map(function(rule){
			for(var key in initialMap) {
				// find initial values for the `key`
				var regex = new RegExp( key + "([^;]*)initial([^;]*)", "g" );

				// while $key: ... initial ...; exists make it $key: ... initialMap[key] ...;
				while( regex.test(rule) ){
					rule = rule.replace(regex, key + "$1" + initialMap[key] + "$2");
				}
			}
